...
def generate_report_from_data(synthesis: SynthesisResponse, output_path: str):
    ...
    fo_counts = {i: {'name': name, 'count': 0, 'percentage': 0} for i, name in fo_names.items()}
    ...
    for finding in context.get('synthesized_findings', []):
        ...
        if cert in certainty_counts:
            certainty_counts[cert] += 1
        else:
            certainty_counts['Ukjent'] += 1
    ...
    evidence_levels = {
        'Nivå 5': {'label': 'Systembaser', 'count': 0, 'color': 'bg-indigo-600', 'p_width': '50%'},
        'Nivå 4': {'label': 'Kliniske oppslagsverk', 'count': 0, 'color': 'bg-indigo-600', 'p_width': '60%'},
        'Nivå 3': {'label': 'Retningslinjer/prosedyrer', 'count': 0, 'color': 'bg-indigo-600', 'p_width': '70%'},
        'Nivå 2': {'label': 'Systematiske oversikter', 'count': 0, 'color': 'bg-indigo-600', 'p_width': '80%'},
        'Nivå 1': {'label': 'Enkeltstudier', 'count': 0, 'color': 'bg-indigo-600', 'p_width': '90%'},
        'Nivå 0': {'label': 'Annet / Ugradert', 'count': 0, 'color': 'bg-indigo-600', 'p_width': '100%'},
    }
    ...
    def get_certainty_desc(level):
        if level == 'Høy': return "Funnene er direkte og entydig beskrevet i kildematerialet, med lite rom for tolkning."
        if level == 'Moderat': return "Funnene er underbygget av kildene, men kan kreve noe faglig tolkning eller kontekstualisering."
